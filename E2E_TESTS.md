# Claude Code Intent E2E Test

End-to-end test for the Claude Code reasoning-plane pipeline: synthetic Claude Code
transcripts are injected, processed by the `claude_code_extrapolator`, and
...
1. **Provision checks** -- installed package layout, version alignment between
   repo and installed copy, MCP snippet presence, PSK file, Claude Code plugin
   registration.
2. **Synthetic transcript generation** -- three Claude Code-style plain text
   transcript files (`e2e_api_*`, `e2e_shell_*`, `e2e_git_*`) are written under
   the Claude projects root.
3. **Extrapolator execution** -- `claude_code_extrapolator.mjs` processes the
...
| `CLAUDE_CODE_EDAMAME_CONFIG` | platform default | Override `config.json` path |
| `E2E_SKIP_PLUGIN_CHECK` | 0 | If 1, skip Claude Code marketplace / MCP path checks |
...
under `tests/e2e/`. Run triggers with `--agent-type claude_code`. See
