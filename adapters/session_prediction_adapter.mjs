...
function looksLikeClaudeCodeProjectPath(filePath, hints) {
...
async function candidateTranscriptFiles(config) {
  const allFiles = await walkFiles(config.claudeCodeProjectsRoot);
  const recencyCutoff = Date.now() - config.transcriptRecencyHours * 60 * 60 * 1000;
  const hintCandidates = allFiles.filter((filePath) =>
    looksLikeClaudeCodeProjectPath(filePath, config.transcriptProjectHints || []),
  );
...
function extractTraffic(text, commands, config) {
  ...
  for (const host of config.claudeCodeLlmHosts || []) {
    if (host.includes(":")) {
      hosts.push(host);
    } else {
      hosts.push(`${host}:443`);
...
export async function collectTranscriptSessions(config, options = {}) {
  ...
  for (const candidate of limited) {
    ...
    records.push({
      ...
      title: firstNonEmptyLine(userText) || `Claude Code session ${candidate.sessionId}`,
...
export async function buildRawSessionIngestPayload(config, options = {}) {
  ...
  const agentInstanceId = config.agentInstanceId || "claude-code-default";
  ...
  const rawSessions = {
    ...
    sessions: sessions.map((session) => {
      ...
      return {
        ...
        derived_expected_traffic: uniqueStrings([...derivedExpectedTraffic, ...(config.claudeCodeLlmHosts || [])]),
