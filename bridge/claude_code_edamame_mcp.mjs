...
const CONTROL_CENTER_TOOL_NAME = "edamame_claude_code_control_center";
const CONTROL_CENTER_REFRESH_TOOL_NAME = "edamame_claude_code_control_center_refresh";
const CONTROL_CENTER_REFRESH_NOW_TOOL_NAME = "edamame_claude_code_control_center_refresh_now";
const CONTROL_CENTER_APPLY_PAIRING_TOOL_NAME = "edamame_claude_code_control_center_apply_pairing";
const CONTROL_CENTER_RUN_HOST_ACTION_TOOL_NAME = "edamame_claude_code_control_center_run_host_action";
const CONTROL_CENTER_REQUEST_APP_PAIRING_TOOL_NAME = "edamame_claude_code_control_center_request_app_pairing";
const REFRESH_TOOL_NAME = "claude_code_refresh_behavioral_model";
const LEGACY_REFRESH_TOOL_NAME = "claude_code.refresh_behavioral_model";
const HEALTHCHECK_TOOL_NAME = "claude_code_healthcheck";
const LEGACY_HEALTHCHECK_TOOL_NAME = "claude_code.healthcheck";
const POSTURE_SUMMARY_TOOL_NAME = "claude_code_posture_summary";
const LEGACY_POSTURE_SUMMARY_TOOL_NAME = "claude_code.posture_summary";

export const TOOL_DEFINITIONS = [
  {
    name: CONTROL_CENTER_TOOL_NAME,
    title: "Claude Code EDAMAME Control Center",
    description:
  ...
  },
  {
    ...
    title: "Refresh Claude Code EDAMAME Control Center",
  ...
  },
  {
    ...
    title: "Apply Claude Code EDAMAME Pairing",
    description:
      "Store the local EDAMAME MCP endpoint and credential (PSK or per-client token) for the Claude Code bridge, then return an updated control-center snapshot.",
    annotations: {
      title: "Apply Claude Code EDAMAME Pairing",
    },
    _meta: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        psk: {
          type: "string",
          description: "EDAMAME MCP credential (PSK or per-client token) to store locally for this Claude Code bridge.",
        },
      },
    },
  },
  {
    ...
    title: "Run Claude Code EDAMAME Host Action",
    description:
  ...
  },
  {
    ...
    description:
      "Read recent Claude Code transcripts, forward raw reasoning sessions to EDAMAME, and let EDAMAME's configured LLM build and upsert the BehavioralWindow.",
    inputSchema: {
      type: "object",
      additionalProperties: false,
      properties: {
  ...
  },
  {
    ...
    description:
      "Check local Claude Code package configuration, EDAMAME MCP reachability, divergence-engine status, and behavioral-model presence.",
    inputSchema: {
      type: "object",
      additionalProperties: false,
      properties: {
...
export const RESOURCE_DEFINITIONS = [
  {
    ...
    name: "Claude Code EDAMAME Control Center",
    description:
      "Interactive setup and status dashboard for Claude Code-to-EDAMAME pairing, extrapolation state, and posture summary.",
...
function backgroundRefreshLoggingEnabled(config) {
  ...
  const envValue = String(process.env.CLAUDE_CODE_EDAMAME_LOG_REFRESH || "")
    .trim()
    .toLowerCase();
...
export function createClaudeCodeDrivenRefresh(config, options = {}) {
  const runExtrapolation = options.runExtrapolation || runLatestExtrapolation;
  ...
  async function maybeRun(trigger, options = {}) {
    ...
    if (!force) {
      const persistedState = await loadStateFn(config, "claude-code-extrapolator", null);
    ...
    const runPromise = (async () => {
      ...
      try {
        ...
        if (logRefresh) {
          stderrLog(
            `claude_code_refresh trigger=${trigger} success=${result?.success !== false} upserted=${result?.upserted === true} sessions=${result?.sessionCount ?? 0} reasons=${formatRefreshReasons(result)}`,
          );
        }
        return { skipped: false, trigger, result };
      } catch (error) {
        if (logRefresh) {
          stderrLog(
            `claude_code_refresh trigger=${trigger} unexpected_error=${String(error?.stack || error)}`,
          );
        }
        return { skipped: false, trigger, error };
      } finally {
...
async function main() {
  ...
  const autoRefresh = createClaudeCodeDrivenRefresh(config);
