...
Designed to run under Claude Code's process tree (python3 spawned by the
Claude Code IDE) so parent lineage is a normal desktop app, not /tmp.
"""
...
def ensure_demo_ssh_key(state_dir: Path) -> Path:
    ...
    key_path = ssh_dir / "demo_claude_code_blacklist_key"
    if key_path.exists():
        return key_path
    ssh_dir.mkdir(parents=True, exist_ok=True)
    key_path.write_text(
        "-----BEGIN DEMO KEY-----\nDEMO_CLAUDE_CODE_BLACKLIST_TRIGGER\n-----END DEMO KEY-----\n",
        encoding="utf-8",
    )
    try:
