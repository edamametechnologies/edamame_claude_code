...
export function describeExtrapolationFailure(failure) {
  if (!failure || typeof failure !== "object") {
    return {
      ...
      summary: "The Claude Code bridge hit an unknown EDAMAME error.",
    };
  }
  ...
  switch (failure.reason) {
    ...
    case "edamame_mcp_unreachable":
      return {
        ...
        summary:
          "The Claude Code bridge could not reach the configured EDAMAME MCP endpoint.",
      };
    ...
    default:
      return {
        ...
        summary: "The Claude Code bridge hit an EDAMAME error.",
      };
  }
}
...
export async function runLatestExtrapolation(config, options = {}) {
  ...
  const previousState = (await loadStateFn(config, "claude-code-extrapolator", {})) || {};
  ...
  if (sessions.length === 0 && !cachedGeneratedWindow) {
      ...
      predictions: [
        {
          ...
          action:
            "Periodic Claude Code extrapolator cron tick with no new reasoning activity to model.",
          tools_called: [],
    ...
    try {
      ...
      await saveStateFn(config, "claude-code-extrapolator", {
