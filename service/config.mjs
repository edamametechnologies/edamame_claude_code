...
export function defaultConfigDir() {
  if (process.platform === "darwin") {
    return path.join(os.homedir(), "Library", "Application Support", "claude-code-edamame");
  }
  ...
  return path.join(process.env.XDG_CONFIG_HOME || path.join(os.homedir(), ".config"), "claude-code-edamame");
}
...
export function defaultSystemctlCommand() {
  if (typeof process.env.CLAUDE_CODE_EDAMAME_SYSTEMCTL_COMMAND === "string") {
    const override = process.env.CLAUDE_CODE_EDAMAME_SYSTEMCTL_COMMAND.trim();
    if (override) return override;
  }
  return "systemctl";
}
...
export const DEFAULT_CONFIG = Object.freeze({
  ...
  claudeCodeProjectsRoot: path.join(os.homedir(), ".claude", "projects"),
  ...
  claudeCodeLlmHosts: [
    ...
    "asn:NOTION",
    "asn:MICROSOFT-CORP",
    // Claude Code LLM traffic routes through AWS (AS14618 AMAZON-AES,
...
function normalizeConfig(raw = {}, overrides = {}) {
  ...
  return {
    workspaceRoot,
    claudeCodeProjectsRoot: path.resolve(
      expandHome(firstDefined(merged, "claudeCodeProjectsRoot", "claude_code_projects_root")) ||
        DEFAULT_CONFIG.claudeCodeProjectsRoot,
    ),
    ...
    claudeCodeLlmHosts: uniqueStrings(
      toArray(firstDefined(merged, "claudeCodeLlmHosts", "claude_code_llm_hosts")).concat(DEFAULT_CONFIG.claudeCodeLlmHosts),
    ),
...
export async function loadConfig(options = {}) {
  const configPath = path.resolve(expandHome(options.configPath || process.env.CLAUDE_CODE_EDAMAME_CONFIG || defaultConfigPath()));
