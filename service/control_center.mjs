...
import { runLatestExtrapolation } from "./claude_code_extrapolator.mjs";
...
function compactHealthDetail(name, detail) {
  ...
  if (name === "claude_code.extrapolator") {
...
function baselineConfigJson(config) {
  return {
    ...
    claude_code_projects_root: config.claudeCodeProjectsRoot,
    ...
    claude_code_llm_hosts: config.claudeCodeLlmHosts,
...
function summaryLine({ pairingConfigured, posture, health, hostActionResult, refreshResult, extrapolator }) {
  ...
  if (!pairingConfigured) return "Pair the local Claude Code bridge with EDAMAME to enable status reads.";
  const authFailure =
...
export async function buildControlCenterPayload(config, options = {}) {
  ...
  const extrapolatorState = compactExtrapolatorState(
    await loadState(config, "claude-code-extrapolator", {}),
  );
  ...
  return {
    ...
    config: {
      ...
      claudeCodeProjectsRoot: config.claudeCodeProjectsRoot,
...
export async function applyPairing(config, args = {}) {
  ...
  try {
    const extState = await loadState(config, "claude-code-extrapolator", {});
    if (
      ...
      await saveState(config, "claude-code-extrapolator", extState);
    }
  } catch (_e) {
