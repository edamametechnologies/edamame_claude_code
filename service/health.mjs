...
} from "./claude_code_extrapolator.mjs";
...
export async function runHealthcheck(config, options = {}) {
  ...
  addCheck("config.claudeCodeProjectsRoot", !!config.claudeCodeProjectsRoot, config.claudeCodeProjectsRoot);
  ...
  const extrapolatorState = await loadState(config, "claude-code-extrapolator", null);
  if (extrapolatorState) {
    const lastError =
      typeof extrapolatorState.lastError === "string" && extrapolatorState.lastError.trim()
        ...
        : null;
    addCheck("claude_code.extrapolator", !lastError, {
