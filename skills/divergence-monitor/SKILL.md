...
description: Monitor and diagnose behavioral divergence between Claude Code intent and observed system activity. Use when the user asks about divergence verdicts, behavioral model status, or wants to debug why EDAMAME flagged an alert.
---
...
- User asks about the behavioral model or what EDAMAME thinks Claude Code is doing
...
1. Start by calling `edamame_claude_code_control_center` to get the full status snapshot including:
...
4. If the verdict is `NO_MODEL`, guide the user to export intent:
   - Use `edamame_claude_code_control_center_refresh_now` to push current transcripts
...
- Multiple agent contributors (Claude Code, Cursor, OpenClaw, etc.) may be merged into one model. The Claude Code slice is identified by `agent_type=claude_code`.
