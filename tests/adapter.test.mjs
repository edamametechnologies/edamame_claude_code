...
async function makeTempFixture() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "claude-code-edamame-adapter-"));
  const workspaceRoot = path.join(root, "edamame_project");
  const siblingRepo = path.join(root, "edamame_core");
  const claudeCodeProjectsRoot = path.join(root, "claude-projects");
  const transcriptDir = path.join(claudeCodeProjectsRoot, "fixture-workspace");
...
`,
  ...
  return { root, workspaceRoot, siblingRepo, claudeCodeProjectsRoot, transcriptDir };
}

function makeBaseConfig(fixture, overrides = {}) {
  return {
    workspaceRoot: fixture.workspaceRoot,
    claudeCodeProjectsRoot: fixture.claudeCodeProjectsRoot,
    transcriptProjectHints: ["fixture-workspace"],
    transcriptLimit: 4,
    transcriptRecencyHours: 48,
    transcriptActiveWindowMinutes: 5,
    agentType: "claude_code",
    agentInstanceId: "claude-code-test-fixture",
...
test("buildRawSessionIngestPayload forwards transcript context with derived session hints", async () => {
  ...
  const result = await buildRawSessionIngestPayload(
    makeBaseConfig(fixture, {
      scopeParentPaths: ["*/Claude*", "*/claude_code_edamame_mcp.mjs"],
    }),
  );

  assert.equal(result.rawSessions.agent_type, "claude_code");
  assert.equal(result.rawSessions.agent_instance_id, "claude-code-test-fixture");
  assert.equal(result.rawSessions.source_kind, "claude_code");
  ...
  assert.deepEqual(rawSession.derived_scope_parent_paths, ["*/Claude*", "*/claude_code_edamame_mcp.mjs"]);
...
test("collectTranscriptSessions extracts tools and commands from JSONL assistant text", async () => {
  ...
  const sessions = await collectTranscriptSessions(
    makeBaseConfig(fixture, { claudeCodeLlmHosts: [] }),
  );
