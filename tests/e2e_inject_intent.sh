#!/usr/bin/env bash
# End-to-end local test: provision check, three synthetic Claude Code transcripts (API / shell / git),
...
#   CLAUDE_CODE_EDAMAME_CONFIG  Override path to claude-code-edamame config.json
#   E2E_SKIP_PLUGIN_CHECK       If set to 1, skip Claude Code marketplace / MCP path checks
...
REPO_ROOT="${CLAUDE_CODE_REPO_ROOT:-$(cd "$SCRIPT_DIR/.." && pwd)}"

OS_KERNEL="$(uname -s)"
case "$OS_KERNEL" in
  Darwin)
    CONFIG_HOME="$HOME/Library/Application Support/claude-code-edamame"
    DATA_HOME="$HOME/Library/Application Support/claude-code-edamame"
    ;;
  MINGW*|MSYS*|CYGWIN*)
    CONFIG_HOME="${APPDATA:-$HOME/AppData/Roaming}/claude-code-edamame"
    DATA_HOME="${LOCALAPPDATA:-$HOME/AppData/Local}/claude-code-edamame"
    ;;
  *)
    CONFIG_HOME="${XDG_CONFIG_HOME:-$HOME/.config}/claude-code-edamame"
    DATA_HOME="${XDG_DATA_HOME:-$HOME/.local/share}/claude-code-edamame"
    ;;
esac

INSTALL_ROOT="$DATA_HOME/current"
CONFIG_PATH="${CLAUDE_CODE_EDAMAME_CONFIG:-$CONFIG_HOME/config.json}"
MCP_SNIPPET_PATH="$CONFIG_HOME/claude-code-mcp.json"
...
if [[ "${E2E_SKIP_PROVISION_STRICT:-0}" != "1" ]]; then
  ...
  if [[ ! -f "$INSTALL_ROOT/service/claude_code_extrapolator.mjs" ]]; then
    echo "FAIL: installed package missing: $INSTALL_ROOT/service/claude_code_extrapolator.mjs" >&2
    echo "Run from edamame_claude_code: bash setup/install.sh <workspace_root>" >&2
...
PY
  ...
  if ! grep -q "claude_code_edamame_mcp" "$MCP_SNIPPET_PATH" 2>/dev/null; then
    echo "FAIL: $MCP_SNIPPET_PATH must reference claude_code_edamame_mcp.mjs" >&2
    exit 1
  fi
...
if [[ ! -f "$PACKAGE_ROOT/service/claude_code_extrapolator.mjs" ]]; then
  echo "FAIL: claude_code_extrapolator.mjs not found under $PACKAGE_ROOT/service" >&2
  exit 1
fi
...
if [[ "${E2E_SKIP_PLUGIN_CHECK:-0}" == "1" ]]; then
  echo "SKIP: Claude Code plugin/MCP verification (E2E_SKIP_PLUGIN_CHECK=1)"
...
" 2>/dev/null; then
  ...
  if [[ "${E2E_SKIP_PROVISION_STRICT:-0}" != "1" ]]; then
    ...
    echo "OK: using MCP snippet (no Claude Code settings.json)"
  fi
fi
...
agent_type = (cfg.get('agent_type') or 'claude_code').strip()
agent_id = (cfg.get('agent_instance_id') or '').strip()
cc_rel = cfg.get('claude_code_projects_root') or cfg.get('claude_projects_root') or '~/.claude/projects'
...
export _E2E_DIAG_SUITE="claude_code"
